...
export default function HomePage() {
  ...
  const GoogleButton = ({ label }: { label: string }) => (
    <button
      ...
      disabled={loading}
      className="w-full flex items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3.5 font-semibold text-slate-800 transition-colors hover:bg-slate-50 disabled:opacity-40"
    >
  ...
  if (mode === 'login') {
    return (
      <AuthFrame title="Bejelentkezés" subtitle="Lépj vissza a rendeléseidhez, játékokhoz vagy az admin felületre." backMode="landing">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button type="button" onClick={() => { setMode('forgot'); setErr(''); setMsg('') }} className="w-full py-1 text-center text-sm text-white/40 transition-colors hover:text-white/70">
            Elfelejtett jelszó?
          </button>
        </form>
        <div className="mt-6 text-center">
          <button onClick={() => { setMode('register'); setErr(''); setMsg('') }} className="text-sm text-amber-400">
  ...
  if (mode === 'register') {
    return (
      <AuthFrame title="Regisztráció" subtitle="Válaszd ki, hogy vendégként vagy venue oldali szereplőként csatlakozol." backMode="landing">
            ...
            return (
              <button
                ...
                className={`rounded-2xl px-4 py-3 text-sm font-semibold transition-all ${
                  active ? 'bg-amber-500 text-black shadow-lg' : 'text-white/60 hover:text-white'
                }`}
              >
        ...
        <div className="mt-6 text-center">
          <button onClick={() => { setMode('login'); setErr(''); setMsg('') }} className="text-sm text-amber-400">
  ...
  return (
    <AuthFrame title="Elfelejtett jelszó" subtitle="Add meg az email címed, és küldünk egy biztonságos visszaállítási linket." backMode="login">
      {msg ? (
